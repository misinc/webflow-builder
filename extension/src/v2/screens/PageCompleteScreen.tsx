...
export function PageCompleteScreen() {
  ...
  const { activeQueue, designerContext, pageProgressRows } = useAppState();
  const builtCount =
    activeQueue?.items.filter((item) => item.status === "approved").length ?? 0;
  const skippedCount =
    activeQueue?.items.filter((item) => item.status === "skipped").length ?? 0;
  const nextPage =
    pageProgressRows.find(
      (page) => !page.active && page.mapped && page.remainingCount > 0
    ) ?? null;
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-[520px] text-center">
          ...
          <h2 className="text-[24px] font-bold text-wb-text-primary tracking-tight mb-2.5">
            {designerContext?.pageName ?? "Current page"} is complete
          </h2>
          <p className="text-[13.5px] text-wb-text-secondary leading-relaxed mb-6">
            Every section on the active mapped page has been addressed in the persisted workflow queue.
          </p>
          ...
          <div className="bg-wb-surface-1 border border-white/[0.09] rounded-[10px] mb-5.5 text-left overflow-hidden">
            <div className="flex items-center gap-3 px-4.5 py-4 border-b border-white/[0.06]">
              ...
              <div className="flex-1">
                <div className="text-[13.5px] font-semibold text-wb-text-primary">
                  {designerContext?.pageName ?? "Current page"}
                </div>
                <div className="text-[11px] text-wb-text-tertiary font-mono mt-0.5">
                  {activeQueue?.repoPage?.sourceFile ?? "No mapped repo page"}
                </div>
              </div>
              <CompleteBadge />
            </div>
            <div className="grid grid-cols-3 py-4">
              <Stat value={String(builtCount)} label="Built" color="text-wb-success" border />
              <Stat value={String(skippedCount)} label="Skipped" color="text-wb-skipped" border />
              <Stat value={String(activeQueue?.items.length ?? 0)} label="Tracked" color="text-wb-text-primary" />
            </div>
          </div>
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-[12.5px] text-wb-text-primary font-medium">
                  {nextPage.webflowPageName} — {nextPage.remainingCount} sections remaining
                </div>
              </div>
            </div>
          ) : null}
