...
import { serializeSectionContext } from "@wfb/backend-core/planner/section-serializer.js";
import type { SectionContext } from "@wfb/shared/contracts.js";
...
const JSX_IMAGE_SECTION = `<section className="bg-white py-20"><div className="max-w-7xl mx-auto px-6 lg:px-8"><h2 className="text-3xl mb-12 text-foreground text-center">Our Attorneys</h2><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div className="lg:col-span-1"><div className="relative aspect-[3/4] mb-6 overflow-hidden rounded"><ImageWithFallback src={markPhoto} alt="Portrait of Mark Windsor" className="w-full h-full object-cover" /></div><h3 className="text-2xl mb-2">Mark Windsor</h3><p className="text-sm text-muted-foreground mb-4">Partner</p></div></div></div></section>`;
const FOOTER_HTML = `<footer class="bg-primary text-primary-foreground py-16 mt-24"><div class="max-w-7xl mx-auto px-6 lg:px-8"><div class="grid grid-cols-1 md:grid-cols-3 gap-12"><div><a class="mb-4 inline-flex rounded bg-white/95 px-4 py-3" href="/"><img src="/assets/windsor-kimball-apc.png" alt="Windsor Kimball APC" class="h-8 w-auto"></a><p class="text-sm text-primary-foreground/80 leading-relaxed">Federal criminal defense attorneys serving clients from investigation through appeal throughout Southern California and the Ninth Circuit.</p></div><div><h4 class="mb-4 text-sm uppercase tracking-wider">Quick Links</h4><ul class="space-y-2 text-sm"><li><a href="/">Home</a></li><li><a href="/about">About</a></li><li><a href="/practice-areas">Practice Areas</a></li><li><a href="/successes">Successes</a></li><li><a href="/faq">FAQ</a></li><li><a href="/contact">Contact</a></li></ul></div><div><h4 class="mb-4 text-sm uppercase tracking-wider">Contact</h4><div class="space-y-2 text-sm text-primary-foreground/80"><p>65 N. Raymond Avenue, Suite 320</p><p>Pasadena, California 91103</p><p class="pt-2">(626) 792-6700</p><p>mark@windsorlaw.us</p><p>katy@windsorlaw.us</p></div></div></div><div class="mt-12 pt-8 border-t border-primary-foreground/20 text-center text-sm text-primary-foreground/60"><p>© 2026 Windsor Kimball APC. All rights reserved. | Attorney Advertising</p></div></div></footer>`;
...
describe("serializeSectionContext", () => {
  ...
  it("treats JSX image components as image nodes in outlines and content extraction", () => {
    const serialized = serializeSectionContext(htmlContext(JSX_IMAGE_SECTION));
    const values = serialized.content.map((item) => item.value);

    expect(serialized.sourceExcerpt).toContain('img "Portrait of Mark Windsor"');
    expect(values).toContain("Portrait of Mark Windsor");
    expect(values).toContain("Mark Windsor");
  });

  it("preserves footer link and contact text items from html", () => {
    const serialized = serializeSectionContext(htmlContext(FOOTER_HTML));
    const values = serialized.content.map((item) => item.value);

    expect(values).toContain("Quick Links");
    expect(values).toContain("Home");
    expect(values).toContain("About");
    expect(values).toContain("Practice Areas");
    expect(values).toContain("Successes");
    expect(values).toContain("FAQ");
    expect(values).toContain("Contact");
    expect(values).toContain("65 N. Raymond Avenue, Suite 320");
    expect(values).toContain("Pasadena, California 91103");
    expect(values).toContain("(626) 792-6700");
    expect(values).toContain("mark@windsorlaw.us");
    expect(values).toContain("katy@windsorlaw.us");
